...
use iced::widget::{Space, button, canvas, container, text, scrollable};
use iced::{Border, Color, Element, Length, Point, Rectangle, Renderer, Theme, border, mouse, font};
...
pub fn modern_scrollable<'a, T: 'a>(content: impl Into<Element<'a, T>>) -> iced::widget::Scrollable<'a, T> {
    scrollable(content)
...
pub fn nav_button<'a>(label: String, route: Route, current_route: &Route) -> Element<'a, Message> {
    ...
    let content = container(
        row![
            ...
            text(icon).size(16),
            Space::new().width(12),
            text(label).size(15).font(font::Font { weight: if is_active { font::Weight::Bold } else { font::Weight::Normal }, ..Default::default() }),
        ]
        ...
        .style(move |_theme, status| {
            ...
            if is_active {
                ...
                style.text_color = if status == button::Status::Hovered {
                    ...
                    if is_dark { Color::from_rgba(1.0, 1.0, 1.0, 0.4) } else { Color::from_rgba(0.0, 0.0, 0.0, 0.5) }
                };
            }
            style
        })
...
pub fn status_dot<'a>(active: bool) -> Element<'a, Message> {
    let color = if active { WEB_SUCCESS } else { Color::from_rgb(0.97, 0.44, 0.44) };
        ...
        .style(|_theme: &Theme| {
            ...
            container::Style {
                ...
                border: Border {
                    ...
                    width: 1.0,
                    color: if is_dark { Color::from_rgba(1.0, 1.0, 1.0, 0.08) } else { Color::from_rgba(0.0, 0.0, 0.0, 0.05) },
                },
                ..Default::default()
            }
        })
...
pub fn premium_card<'a, Message: 'a>(content: impl Into<Element<'a, Message>>) -> Element<'a, Message> {
    container(content)
        .width(Length::Fill)
        .style(|_theme: &Theme| {
            container::Style {
...
pub struct TrafficChart { pub history: VecDeque<(u64, u64)> }

impl<Message> canvas::Program<Message> for TrafficChart {
    type State = ();
    fn draw(&self, _state: &(), _renderer: &Renderer, _theme: &Theme, bounds: Rectangle, _cursor: mouse::Cursor) -> Vec<canvas::Geometry> {
        let mut frame = canvas::Frame::new(_renderer, bounds.size());
        if self.history.len() < 2 { return vec![frame.into_geometry()]; }
        ...
        let down_path = canvas::Path::new(|p| {
            ...
            for (i, (_, down)) in self.history.iter().enumerate() { p.line_to(Point::new(i as f32 * x_step, scale(*down))); }
        ...
        frame.stroke(&down_line, canvas::Stroke::default().with_color(WEB_ACCENT).with_width(2.5));
        ...
        frame.stroke(&up_line, canvas::Stroke::default().with_color(WEB_SUCCESS).with_width(2.0));
