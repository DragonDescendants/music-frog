...
use iced::widget::{Canvas, Space, button, column, container, row, text};
...
pub fn view(state: &AppState) -> Element<'_, Message> {
    ...
    let core_control = card(column![
        ...
        row![
            column![
                ...
                row![
                    text(state.proxy_mode.as_deref().unwrap_or("rule"))
                        .size(12)
                        .style(|_theme| text::Style {
                            color: Some(Color::from_rgb(0.4, 0.4, 0.4))
                        }),
            ...
            if state.runtime.is_some() {
                ...
                button(text(lang.tr("status_starting")).size(12)).padding([8, 16])
            }
        ]
        .align_y(Alignment::Center)
    ]);
    ...
    let proxy_node_section: Element<'_, Message> = if let Some(_rt) = &state.runtime {
        let current_node = global_proxy
            .and_then(|p| p.now.as_ref())
            .cloned()
    ...
    column![
        ...
        if let Some(err) = &state.error_msg {
            column![
                Space::new().height(20),
                container(text(format!("Error: {}", err)).style(|_theme| text::Style {
